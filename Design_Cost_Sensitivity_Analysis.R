...
sens_results$designcostpercentage <- rep(c("0%", "25%", "50%", "75%", "100%"), each =4)
sens_results$designcostpercentage <- factor(sens_results$designcostpercentage, levels = c("100%", "75%", "50%", "25%", "0%"))

sens_results$scenario <- rep(c("Flexibility", "6 Month Delay", "12 Month Delay"), each = 20)

sens_results$lines    <- rep(c("Standard 0%",   "Simple 0%",   "Advanced 0%",   "Robust 0%", 
                               "Standard 25%",  "Simple 25%",  "Advanced 25%",  "Robust 25%", 
                               "Standard 50%",  "Simple 50%",  "Advanced 50%",  "Robust 50%", 
                               "Standard 75%",  "Simple 75%",  "Advanced 75%",  "Robust 75%", 
                               "Standard 100%", "Simple 100%", "Advanced 100%", "Robust 100%"))
...
sens_results$ArmorLabel <- ifelse(sens_results$armortype == 1, "Standard", 
                                  ifelse(sens_results$armortype == 2, "Simple Add-On", 
                                         ifelse(sens_results$armortype == 3, "Advanced Add-On", 
                                                ifelse(sens_results$armortype == 4, "Robust", NA))))

sens_results$ArmorLabel <- factor(sens_results$ArmorLabel, levels = c("Standard", "Simple Add-On", "Advanced Add-On", "Robust"))

cdplot <- ggplot(subset(sens_results, armortype == 1 | armortype ==2 | armortype == 3 | armortype == 4), aes(x = scenario, y = ENPC, group = lines, color = designcostpercentage)) + 
  geom_line(aes(color = designcostpercentage, linetype = designcostpercentage)) + 
  ...
  facet_wrap(facets = "ArmorLabel", ncol = 4) #+
