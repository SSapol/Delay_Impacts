...
flex_0      <- simruns(M, 0, 1,1,1,0)
flex_25     <- simruns(M, 0.25, 1,1,1,0)
flex_50     <- simruns(M, 0.5, 1,1,1,0)
flex_75     <- simruns(M, 0.75, 1,1,1,0)
flex_100    <- simruns(M, 1, 1,1,1,0)

delay6_0      <- simruns(M, 0, 1,1,1,6)
delay6_25     <- simruns(M, 0.25, 1,1,1,6)
delay6_50     <- simruns(M, 0.5, 1,1,1,6)
delay6_75     <- simruns(M, 0.75, 1,1,1,6)
delay6_100    <- simruns(M, 1, 1,1,1,6)

delay12_0      <- simruns(M, 0, 1,1,1,12)
delay12_25     <- simruns(M, 0.25, 1,1,1,12)
delay12_50     <- simruns(M, 0.5, 1,1,1,12)
delay12_75     <- simruns(M, 0.75, 1,1,1,12)
delay12_100    <- simruns(M, 1, 1,1,1,12)
...
ggplot(subset(sens_results, armortype == 2 | armortype == 3), aes(x = scenario, y = diff, group = as.factor(armortype), color = as.factor(armortype))) + 
